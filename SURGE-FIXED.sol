...
contract Reflective is IERC20, Authorizable {
    ...
    address payable public constant reserve = payable(INSERT RESERVE ADDRESS); 
    ...
    function getMinimumAccumulationForBooster() public view returns (uint256) {
        return _minimumAccumulationForBooster;
    }
    function getMinimumAccumulationForLiquidity() public view returns (uint256) {
...
contract Token is Reflective {
    ...
    function setCP(uint8 amount) external onlyAuthorized returns (bool) {
        require(amount <= 1000, //support for 0.X %, max of 10 % remains.
        "must not exceed 10 percent");
    ...
    function getCycledLP() public view returns (uint256) {
        return (getRemoveableLP().mul(getUP())).div(_DENOMINATOR);
    }
    ...
    function getUP() public view returns (uint8) {
        return _up;
    }
    function getCP() public view returns (uint8) {
        return _cp;
    }
    function getRP() public view returns (uint8) {
        return _rp;
    }
    ...
    function getLockableTokens() private view returns (uint256) {
...
contract Outpost is Context {
    ...
    uint256 private constant _DENOMINATOR = 100;
    address payable private constant _RESERVE = "INSERT RESERVE ADDRESS";
